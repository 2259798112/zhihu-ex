...
function parseInitData(json,id) {
    ...
    questionInfoTag(span,'天',days);
    questionInfoTag(span,'回答',answerCount);
    questionInfoTag(span,'日增',dv);
    questionInfoTag(span,'浏览',visitCount);
    questionInfoTag(span,'5答',defaultAnswerVoteCount);
    return span;
}
...
function qInfo() {
    ...
    qList.forEach(item =>{let url = item.querySelector('meta[itemprop="url"]').content;
        console.log(url);
        parse(url);
    })
